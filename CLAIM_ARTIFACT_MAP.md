...
The current abstract centers prior-controlled hierarchy reconstruction and
the paired APT increment over RNA references. Split sensitivity is historical
appendix background only; its Route A audit and artifact limitations are in
`analysis/SPLIT_AUDIT_RESOLUTION.md`. The split-control evidence is in
