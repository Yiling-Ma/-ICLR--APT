...
本轮统一协议训练及 130/130 预测产物核查已完成，主表改用统一 v2 结果。历史主表单独保留在附录，两项已完成诊断不与新模型混用。
...
## 2026-09-14 完成核查与论文更新

两台主机训练结束；完整 OOF、原始 ID/标签、各阶段计数、训练集 scaler 及选择历史核查通过。发现 HCE 初始 coarse 导出未使用 subtree mass，已用 15 个冻结 checkpoint 补推理并保存原导出，fine argmax 完全一致；本次整理新增训练为 0。最终结果、配对区间和论文表格从修正后产物生成，详见 RESULTS_UPDATE.md。

未实施 E0 因素网格、强 RNA/E2、E3 额外 shuffle、独立队列等不同任务。当前冻结 raw-count 定义不基于 outer 分数选取。CPU/GPU 时间仅作分项复现记录，不直接排名。区间未证明 cascade 优势或模型等效。
