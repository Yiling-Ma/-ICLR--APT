...
comparisons measure their practical tradeoff, but do not isolate subject
breadth from changes in training-label support and composition. We distinguish
total allocation effects from label-standardized effects using APT-Bench, a patient-disjoint
...
held-out subjects to assess stability. Breadth estimates become smaller and
policy-dependent; fine-resolution point estimates reverse for both linear and
boosted-tree models in APT and COMBAT. These joint stability ranges are not
confidence intervals for the seed-averaged effect. Controlled simulations
check recovery of a specified allocation target against exact-null and
independent Monte Carlo references. The real-cohort comparison changes both
class quotas and donor allocation, so it does not identify their separate
contributions. APT-Bench makes the sampling policy, label distribution, and
stability of an allocation decision explicit, without equating practical
recruitment gains with an isolated subject-breadth effect.
\end{abstract}
