...
\title{APT-Bench: Auditing Label-Distribution \mbox{Confounding} in Subject--Cell Scaling}
...
\begin{abstract}
Single-cell learning has two sampling axes: the number of biological subjects
and the number of cells measured within each subject. Fixed-total-cell
comparisons appear to isolate their tradeoff, yet increasing subject breadth
can simultaneously change label support, per-class counts, and class
proportions. We audit this failure mode with APT-Bench, a patient-disjoint
testbed of 361{,}792 aptamer profiles from 40 patients, and external evaluations
on COMBAT and OneK1K. Our protocol fixes observed classes and exact cell quotas,
uses within-class donor balancing, and jointly resamples training subsets and
held-out subjects. The apparent breadth advantage in label-agnostic experiments
attenuates under this protocol; fine-resolution point estimates reverse for
both linear and boosted-tree models in APT and COMBAT, while joint intervals
leave the corrected effects unresolved. Controlled simulations distinguish an
exactly zero allocation effect from independently estimated nonzero targets.
These findings do not establish a universal causal effect of subject breadth:
the real-cohort control changes both label quotas and within-class allocation.
Instead, APT-Bench makes training-label distribution and sampling uncertainty
explicit parts of the evaluation contract, showing why fixed cell totals alone
cannot justify conclusions about subject--cell scaling.
\end{abstract}
...
$T=3{,}200$ and $T=6{,}400$. To test whether the corrected direction is specific
...
\caption{Joint uncertainty for the completed class-matched protocol at
$T=6{,}400$. Points are mean changes in subject-balanced pooled OOF macro-F1
from 8 to 32 subjects. The 95\% joint intervals select a training-subset seed
...
variability. Zero inclusion does not establish equivalence.}
\label{fig:class_matched_joint}
\end{figure}

The primary interval propagates both uncertainty sources. In each of 2{,}000
...
$P=32$, and recompute the paired difference. All 18 joint 95\% intervals
include zero, and the table reports the corresponding $\Pr(\Delta>0)$. For
...
exclude zero even when the joint interval does not. We therefore treat the
joint interval as primary and interpret the conditional intervals only as
diagnostics of the two uncertainty sources.
...
not the primary joint uncertainty. The paired joint effect intervals are
...
to $+0.00123$, and every joint interval includes zero. The four direct OneK1K
$32\rightarrow128$ contrasts are likewise unresolved. This does not by itself
...
\subsection{Practical-Equivalence Sensitivity}
...
For each released joint 95\% interval $[L,U]$, we report zero inclusion
($L\leq0\leq U$) and strict containment ($-\epsilon<L$ and $U<\epsilon$)
separately. Containment is evidence of practical equivalence only relative to
that margin, interval construction, and estimand; zero inclusion alone is not.
Intervals with $L\leq-\epsilon$ and $U\geq\epsilon$ lack precision to exclude
meaningful effects in either direction. Other non-contained intervals can
still leave a meaningful one-sided effect unresolved. These are not mutually
exclusive categories, and interval width alone does not establish formal
statistical power; we therefore use ``insufficient precision'' rather than
claiming a quantified underpowered design.
...
contained. Conclusions about practical equivalence are thus explicitly
margin-dependent, not a blanket finding that subjects have no value.
...
released joint 95\% intervals. Moreover, those intervals sample one completed
