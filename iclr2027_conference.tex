...
\title{APT-Bench: Patient-Disjoint Hierarchical Cell Typing and Novelty-Aware Disease Characterization from Single-Cell Aptamer Profiles}
...
\begin{abstract}
Single-cell aptamer (APT) profiling offers a blood-based view of both disease-associated molecular state and circulating immune composition, but evaluating such data requires separating targets that live at different statistical units. Disease is a patient-level label, whereas lineage and subtype are cell-level labels arranged in a genuine hierarchy. We introduce APT-Bench, a patient-disjoint evaluation setting for multi-resolution characterization on a real-world APT liquid-biopsy cohort of 40 patients and more than 370,000 cells, spanning patient-level disease recognition, cell-level lineage and subtype annotation, leave-one-disease-out novelty detection, protocol-sensitivity analysis, and measurement-efficiency analysis.

Across this benchmark, within-cohort disease recognition is extremely strong under patient-disjoint folds, but stress tests show that this signal depends heavily on patient-specific absolute-expression structure and therefore should not be overinterpreted as solved clinical generalization. The harder unresolved task is cell-level hierarchy prediction: coarse lineage recognition is consistently easier than fine subtype recognition, and cell-level split protocols substantially overstate performance relative to patient-disjoint evaluation. To instantiate this setting, we introduce \ourmethod{}, a soft-routed lineage-subtype cascade with a trained novelty head. Under patient-level 5-fold evaluation, \ourmethod{} improves coarse-lineage macro-F1 over XGBoost from 0.322 to 0.333 with a paired patient-bootstrap confidence interval excluding zero, while matching but not significantly exceeding XGBoost on fine-subtype macro-F1 (0.156 versus 0.143). Under nested leave-one-disease-out evaluation, the trained novelty head reaches mean AUROC 0.812 and mean AUPR 0.856, outperforming a training-free energy baseline at mean AUROC 0.627. These results position APT-Bench not as a solved diagnostic problem, but as a realistic benchmark for patient-independent hierarchical prediction, explicit novelty awareness, and disciplined interpretation of small-cohort clinical machine learning.
\end{abstract}
...
The resulting development-selected 10-aptamer panels reproduce the full-panel patient-level macro-F1 of 1.0 for both Logistic Regression and XGBoost on all five outer test folds. Figure~\ref{fig:panel_curve} additionally shows the broader budget curve: top-ranked panels improve rapidly with panel size, but random panels also approach full performance as the number of retained aptamers increases, and removing the top-ranked aptamers often leaves performance nearly unchanged. We therefore interpret the compact-panel result as evidence of measurement efficiency rather than as proof that disease information is concentrated in a unique indispensable biomarker set.
...
\caption{Patient-level disease macro-F1 as a function of aptamer budget under the nested panel-selection analysis. Top-ranked panels saturate quickly, but random panels also approach full performance as the budget increases, indicating that predictive signal is distributed across the APT panel rather than concentrated in a unique minimal biomarker set.}
