...
reallocations favor 32 rather than 8 subjects in 37 of 38 APT, COMBAT, and
OneK1K comparisons, but they also increase fine-label coverage. A class-matched
control reveals a resolution-dependent result: modest coarse gains persist in
APT and OneK1K, whereas fine effects are small and change sign across cohorts.
Strict nested
...
\subsection{Class-Coverage Audit and Matched Control}
\label{sec:appendix:class_matched_scaling}

The label-agnostic grid samples cells uniformly within subjects and therefore
permits rare classes to be absent. Machine-readable coverage tables report the
...
slightly with $P$ despite fixed $T$.

The class-matched control removes this pathway. The $P=8$ subset defines the
...
effect. Empirical 95\% subset-seed intervals include zero in 11 of 12 rows; the
exception is APT fine at $T=6{,}400$, which is negative
($[-0.0084,-0.0022]$). The bootstrap and seed intervals answer different
