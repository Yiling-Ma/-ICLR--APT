...
\begin{abstract}
Single-cell clinical assays can contain many cells but few independent subjects,
making the split unit and sampling budget central to evaluation. We introduce
APT-Bench, a patient-disjoint benchmark of 361{,}792 single-cell aptamer profiles
from 40 patients, with a fixed five-lineage/27-subtype ontology, patient-level
characterization audit, and explicit patient and cell training axes. Across
neural and classical references, coarse lineage recognition remains
substantially easier than fine subtype prediction. Unmatched fixed-total
reallocations confound subject breadth with the training label distribution:
although 37 of 38 comparisons favor 32 rather than 8 subjects, a class-matched
control reduces this apparent advantage to small, model- and cohort-dependent
coarse point estimates and negative or near-zero fine point estimates. After
jointly resampling training subsets and held-out subjects, all corrected
intervals include zero. Thus the
unmatched result is an audit target, not evidence for a universal
subject-dominant scaling law. Here, the label-distribution effect includes
changes in observed classes, per-class counts, and class proportions.
Strict nested
aggregation shows that cell-model outputs retain disease-associated variation,
but subtype composition has no resolved increment over lineage, cell count, or
recorded RNA-QC controls. Direct APT discrimination is also sensitive to
patient-specific absolute-expression structure whose biological and technical
origins are not identifiable here. APT-Bench makes statistical unit,
granularity, and training scale explicit for evaluation on unseen subjects.
\end{abstract}
