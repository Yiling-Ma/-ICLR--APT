...
\begin{abstract}
Single-cell aptamer profiling offers a blood-derived molecular input for
immune-cell identification, but evaluating it requires separating performance
on additional cells from generalization to new patients. We introduce
APT-Bench, a benchmark of 361{,}792 cells with 293 aptamer measurements from
40 participants across five cancer groups and healthy controls. Paired
transcriptomic annotations define five lineages and 27 subtypes. Five fixed
patient-disjoint folds and subject-balanced evaluation compare linear,
tree-based, and Transformer references on the same APT-only tasks. We describe
a Soft-cascade Transformer reference that combines global subtype prediction
with softly routed lineage experts, without assuming a model-specific
advantage. The benchmark exposes substantial remaining difficulty in fine
typing; a separate matched-input RNA diagnostic contextualizes, rather than
replaces, the APT comparison. As a downstream case study, nested cell-output
aggregation recovers patient subtype composition better than a training-mean
reference, while incremental disease-prediction value remains unresolved.
APT-Bench establishes a framework for comparing models of this measurement
modality and studying their patient-level use, with clinical interpretation
requiring acquisition-aware external validation.
\end{abstract}
