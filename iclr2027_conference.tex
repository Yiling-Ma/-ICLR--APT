...
coarse effects and negative or near-zero fine effects. The fine-effect reversal
in APT and COMBAT is reproduced by both Logistic Regression and XGBoost, whereas
OneK1K remains near zero. Thus the
...
The table reports paired subject-bootstrap intervals for the seed-averaged
effect. Empirical 95\% subset-seed intervals include zero in 16 of 18 rows. The
two exceptions are the LR and XGBoost APT fine results at $T=6{,}400$, both of
which are negative ($[-0.0084,-0.0022]$ and
$[-0.0101,-0.0017]$, respectively). The bootstrap and seed intervals answer different
questions: the former varies held-out subjects after averaging training-subset
seeds, whereas the latter varies selected development subsets with the test
subjects fixed. We therefore interpret sign and magnitude descriptively rather
than treating either interval family as a universal significance test.
