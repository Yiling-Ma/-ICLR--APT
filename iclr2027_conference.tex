...
Across this benchmark, coarse lineage recognition is consistently easier than fine subtype recognition, revealing a granularity gap that is hidden by aggregate reporting. A hierarchy-aware reference model, \ourmethod{}, improves coarse-lineage macro-F1 over XGBoost from 0.322 to 0.333 with a paired patient-bootstrap confidence interval excluding zero, while remaining competitive but not significantly superior on fine-subtype macro-F1 (0.156 versus 0.143). Patient-level disease recognition is extremely strong under the prespecified patient-disjoint folds, with all five classical baselines correctly assigning all 40 patients by majority vote, but stress tests show that this signal depends heavily on patient-specific absolute-expression structure and therefore should not be overinterpreted as solved clinical generalization. Finally, a nested patient-level panel-selection analysis shows that all five folds select the same development tolerance budget, $B^*=10$, and the resulting 10-aptamer panels preserve the observed full-panel patient macro-F1 of 1.0 for both Logistic Regression and XGBoost; equal-cell subsampling also preserves patient-level accuracy with as few as 10 cells per patient. These results position APT-Bench as a benchmark for patient-independent hierarchical cell typing, disciplined disease-level audit, and measurement-efficient APT analysis in small-cohort clinical machine learning.
\end{abstract}
...
We treat disease-level novelty as an exploratory appendix track rather than a main contribution. In this protocol, all patients from one disease category are withheld during training and evaluated as a disease-level distribution shift. This is useful for stress-testing representations, but it is easier to attack than the main benchmark because the cohort contains only six disease groups and the proxy-novel training rule has limited diversity.

We compare two standard training-free scores, maximum softmax probability \citep{hendrycks2016baseline} and energy \citep{liu2020energy}, with a trained novelty head. For each held-out disease, the cascade is trained only on the known diseases. The encoder is then frozen, and a lightweight MLP novelty head is trained to distinguish one proxy-novel known disease from the remaining known diseases. The truly held-out disease is never used during either cascade training or novelty-head training.

\input{tables/LOD}
\input{tables/novelty}

Under this exploratory nested leave-one-disease-out protocol, the trained novelty head reaches mean AUROC 0.812 and mean AUPR 0.856 across the six held-out disease groups, compared with mean AUROC 0.627 for the strongest training-free energy score. These numbers suggest that disease-level novelty detection is promising, but we do not use them as a headline claim for APT-Bench.
