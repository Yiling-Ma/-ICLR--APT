...
\paragraph{Completed matched-input follow-up.}
...
representation rather than uniquely aptamer-specific biology. The trigger
was not met: the overall conditional-shuffle interval includes zero.
Accordingly, no targeted 5{,}000-HVG conditional shuffle was run; the
independent 5{,}000-HVG reference grid was completed in full.
...
\caption{Two distinct questions under the matched 2,000-HVG probe. Left:
APT oracle minus the conditional training-proportion expected-CM baseline.
Right: paired RNA+APT minus patient-by-lineage-shuffled APT. All lineages
are retained, with fixed child classes and subject-balanced scoring.
Bars are exploratory pointwise joint seed/patient intervals, also resampling
shuffle draws within seed on the right. Axes use different scales; no
cross-panel effect-size equivalence is implied. Oracle information is
privileged and the overall right-panel intervals include zero.}
\label{fig:matched_identity_controls}
\end{figure}

\paragraph{Retrospective rescue and damage.}
For MLP, mean patient accuracy on RNA-error cells improves by 0.311
($[0.291,0.331]$), while accuracy on RNA-correct cells falls by 0.102
($[-0.115,-0.091]$). Against the conditional shuffle, the corresponding
differences are 0.0077 ($[0.0032,0.0124]$) and approximately zero
($[-0.0035,0.0033]$). These exploratory stratum contrasts do not override
the unresolved overall fine-F1 contrast or the unmet follow-up gate.
Complete scores, scope coverage, all seeds, and 288 contrast/metric rows
are retained in the aggregate release. All 30 targeted blocks, 300 fits at
each RNA width, 30 full-budget MLP fits, and 15 frozen neural oracle replays
completed their artifact checks. Six unit tests cover the diagnostic evaluator.
