...
reconstruct the all-cell score in Table~\ref{tab:main}.
...
Within-lineage APT/RNA oracle scores vary substantially: 0.849/0.839 for NK
...
0.490/0.487). These settings do not isolate the effect of budget alone.
...
are supporting evidence in Appendix~\ref{sec:appendix:full_budget_hierarchy}.
...
to 0.607 for RNA+APT: $+0.018$ (joint 95\% CI $[0.009,0.030]$).
At 5,000 training-selected HVGs, it increases from 0.596 to 0.612:
$+0.016$ ($[0.009,0.029]$). SGD also improves at both widths
...
Lineage and RNA-difficulty breakdowns remain exploratory supporting analyses
(Appendix~\ref{sec:appendix:identity_questions}).

A separate composition-recovery case study is retained in
Appendix~\ref{sec:appendix:composition_recovery}. It shows no resolved advantage
over direct median-APT ridge regression and no resolved lineage improvement;
it does not support an additional disease-characterization claim.
