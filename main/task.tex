...
\citep{mcginnis2019doubletfinder,zhang2024pan,yang2024pan}, yielding the operational
labels used throughout this work. A subsequent marker-based annotation audit had
three additional reviewers independently propose a lineage and subtype call, with
supporting positive- and negative-marker evidence, for all 28 reviewed clusters.
Broad lineage assignment was concordant across all three reviewers for every
cluster; fine-grained subtype calls were less stable, particularly among
T-cell activation/differentiation states and B-cell differentiation stages
(Appendix~\ref{sec:appendix:dataset_documentation}). These annotations
define \numlineages{} lineages and \numsubtypes{} subtypes. They remain operational reference
labels, not independently measured biological truth, and evaluation against this
subtype ontology is conditional on that choice.
\todocheck{Double check the annotation audit numbers reported here against the
source spreadsheet before submission.}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/collect_data.pdf}
\caption{\textbf{Workflow for collecting paired single-cell RNA--aptamer data for
\ourbench{}.} Peripheral blood specimens from a multi-cancer and healthy-control
cohort are processed into single-cell suspensions, quality controlled, and
profiled with barcode-linked RNA and \numaptfeatures{}-aptamer measurements from
the same cells. The resulting paired matrices and transcriptome-derived labels
define the patient-disjoint benchmark construction.}
...
All unified main-table recipes use cell-identity supervision only.
Disease-based contrastive supervision is evaluated separately for Flat and
Soft-cascade; it is not an input required at inference.
...
and fitting use development patients. The unified main comparison uses raw
APT counts followed by log1p and fit-partition standardization. The separately
retained historical provided-expression diagnostics have an upstream
transformation provenance limitation
...
Macro-F1 and from mean within-subject Macro-F1. Unified models use this scoring
convention for validation and final reporting. Historical frozen references
are retrospectively re-scored without checkpoint reselection.

Main-table intervals use 5{,}000 resamples and explicitly label their target:
P denotes patient uncertainty for deterministic LR, and SP additionally
resamples the three training seeds of each randomized model. Each SP replicate averages three
...
Appendix~\ref{sec:appendix:unified_v2}. Historical and paired-input diagnostic
intervals retain their separately specified resampling protocols.
